...
class ftp {
	...
	public function logDataToFiles()
	{
		...
		$log['error'] = (isset($log['error']) ? $log['error'] : array('Keine Fehler'));
		$log['ok'] = (isset($log['ok']) ? $log['ok'] : '');

		$sep = "\n".date('y-m-d H-i-s').' ';
		$logc = date('y-m-d H-i-s').' '.join($sep,$log['error'])."\n";
		$this->addToFile($this->logPath.'/sync-error.log',$logc);
		$logc = date('y-m-d H-i-s').' '.join($sep,$log['ok'])."\n";
		$this->addToFile($this->logPath.'/sync-ok.log',$logc);
	}
	 ...
	 * @return	void
	 */
	function addToFile($file, $ins, $mode = 'w+')
	{
		$fp = fopen($file, $mode);
		fwrite($fp,$ins);
		fclose($fp);
